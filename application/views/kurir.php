...
function getOrigin(origin,des,qty,cour) {
  ...
  $.getJSON("tarif/"+origin+"/"+des+"/"+qty+"/"+cour, function(data){     
    $.each(data, function(i,field){  
      ...
      {
          x += "<p class='mb-0'><b>" + field.costs[i].service + "</b> : "+field.costs[i].description+"</p>Ongkos : ";

           for (j in field.costs[i].cost) {
                x += field.costs[i].cost[j].value +"<br>Estimasi Hari : "+field.costs[i].cost[j].etd+"<br>"+field.costs[i].cost[j].note+"<br>";
            }
